...
# Table 1: Current State of every Control (For Dashboard)
...
# Table 2: Full History of every Test Run (For Audit Trail)
...
def db_get_history(cid):
    # Fetch ALL records for this control, newest first
...
# --- 6. PAGE: CONTROL TESTING (History Enforced) ---
def render_testing():
        ...
        if f:
            ...
            if st.button("Run AI Audit"):
                with st.spinner("Analyzing..."):
                    ...
                    # Update DB (Filename is saved to History)
                    db_update_audit(selected_id, status, res, f.name)
                    st.rerun()

        # 3. LATEST RESULT (Visual Feedback)
    ...
    # 4. AUDIT HISTORY (ALL RECORDS)
    st.markdown(f"### 📜 Complete Audit History for {selected_id}")
    ...
    if not history_df.empty:
        ...
        st.info("No tests have been run for this control yet.")

# --- 7. PAGE: DASHBOARD (Visuals) ---
def render_dashboard():
    ...
    with col1:
        st.subheader("Control Status Breakdown")
    ...
    with col2:
        st.subheader("Risk Exposure by Domain")
        # Creating dynamic mock data based on status
        risk_val = 80 if failed > 0 else 20
        domain_data = pd.DataFrame({
            "Domain": ["Identity & Access", "Data Encryption", "Logging & Monitoring", "Network Security"],
            "Risk Score": [risk_val, 30, 45, 10]
        })
        fig2 = px.bar(domain_data, x="Domain", y="Risk Score", color="Risk Score", color_continuous_scale="Reds")
...
with st.sidebar:
    ...
    # ORDER: Ingestion -> View -> Test -> Dashboard
